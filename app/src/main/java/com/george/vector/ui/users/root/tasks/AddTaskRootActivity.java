...
public class AddTaskRootActivity extends AppCompatActivity implements BottomSheetAddImage.StateListener {
    ...
    ActivityResultLauncher<String> selectPictureLauncher = registerForActivityResult(new ActivityResultContracts.GetContent(),
            uri -> {
                fileUri = uri;
                binding.imageTask.setImageURI(fileUri);
            });
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding.topAppBarNewTaskRoot.setNavigationOnClickListener(v -> onBackPressed());

        binding.addExecutorRoot.setOnClickListener(v -> showAddExecutorDialog());

        binding.cardImage.setOnClickListener(v -> addImage.show(getSupportFragmentManager(), "BottomSheetAddImage"));

        binding.doneTaskRoot.setOnClickListener(v -> {
            address = Objects.requireNonNull(binding.textInputLayoutAddressRoot.getEditText()).getText().toString();
            floor = Objects.requireNonNull(binding.textInputLayoutFloorRoot.getEditText()).getText().toString();
            cabinet = Objects.requireNonNull(binding.textInputLayoutCabinetRoot.getEditText()).getText().toString();
            letter = Objects.requireNonNull(binding.textInputLayoutCabinetLiterRoot.getEditText()).getText().toString();
            name_task = Objects.requireNonNull(binding.textInputLayoutNameTaskRoot.getEditText()).getText().toString();
            comment = Objects.requireNonNull(binding.textInputLayoutCommentRoot.getEditText()).getText().toString();
            date_complete = Objects.requireNonNull(binding.textInputLayoutDateTaskRoot.getEditText()).getText().toString();
            email_executor = Objects.requireNonNull(binding.textInputLayoutExecutorRoot.getEditText()).getText().toString();
            status = Objects.requireNonNull(binding.textInputLayoutStatusRoot.getEditText()).getText().toString();
            urgent = binding.urgentRequestCheckBox.isChecked();
            full_name_executor = binding.textInputLayoutFullNameExecutorRoot.getEditText().getText().toString();
    ...
    void showAddExecutorDialog() {
        ...
        adapter.setOnItemClickListener((documentSnapshot, position) -> {
            ...
            documentReference.addSnapshotListener((value, error) -> {
                ...
                Objects.requireNonNull(binding.textInputLayoutExecutorRoot.getEditText()).setText(email_executor);
                Objects.requireNonNull(binding.textInputLayoutFullNameExecutorRoot.getEditText()).setText(full_name_executor);
    ...
    void initFields(String location) {
        ...
        if (location.equals(OST_SCHOOL)) {
            ...
            binding.addressAutoCompleteRoot.setAdapter(adapter);

            binding.textInputLayoutAddressRoot.getEditText().addTextChangedListener(new TextValidator(binding.textInputLayoutAddressRoot.getEditText()) {
                @Override
                public void validate(TextView text_view, String address) {
                    ...
                    switch (address) {
                        ...
                        case "Авиаторов 9. Начальная школа":
                            ...
                            binding.floorAutoRoot.setAdapter(adapter);
                        ...
                        case "Авиаторов 9. Старшая школа":
                            ...
                            binding.floorAutoRoot.setAdapter(arrayHighSchool);
        ...
        if (location.equals(BAR_SCHOOL))
            Objects.requireNonNull(binding.textInputLayoutAddressRoot.getEditText()).setText(getText(R.string.bar_school_address));
        ...
        binding.statusAutoCompleteRoot.setAdapter(adapter_status);
        ...
        binding.literAutoCompleteRoot.setAdapter(adapter_letter);
        ...
        binding.editTextDateTaskRoot.setOnClickListener(v -> new DatePickerDialog(AddTaskRootActivity.this, date, datePickCalendar
    ...
    void setUpHighSchoolCabinets() {
        binding.textInputLayoutFloorRoot.getEditText().addTextChangedListener(new TextValidator(binding.textInputLayoutFloorRoot.getEditText()) {
            @Override
            public void validate(TextView text_view, String floor) {
                ...
                switch (floor) {
                    ...
                    case "1":
                        ...
                        binding.cabinetAutoRoot.setAdapter(arrayCabinetsFist);
                        break;
                    ...
                    case "2":
                        ...
                        binding.cabinetAutoRoot.setAdapter(arrayCabinetsSecond);
                        break;
                    ...
                    case "3":
                        ...
                        binding.cabinetAutoRoot.setAdapter(arrayCabinetsThird);
                        break;
    ...
    void setDate() {
        ...
        Objects.requireNonNull(binding.textInputLayoutDateTaskRoot.getEditText()).setText(sdf.format(datePickCalendar.getTime()));
    }

    boolean validateFields() {
        Utils utils = new Utils();

        utils.clear_error(binding.textInputLayoutAddressRoot);
        utils.clear_error(binding.textInputLayoutFloorRoot);
        utils.clear_error(binding.textInputLayoutCabinetRoot);
        utils.clear_error(binding.textInputLayoutNameTaskRoot);
        utils.clear_error(binding.textInputLayoutDateTaskRoot);
        utils.clear_error(binding.textInputLayoutExecutorRoot);
        utils.clear_error(binding.textInputLayoutStatusRoot);
        utils.clear_error(binding.textInputLayoutFullNameExecutorRoot);

        boolean check_address = utils.validate_field(address, binding.textInputLayoutAddressRoot);
        boolean check_name_task = utils.validate_field(name_task, binding.textInputLayoutNameTaskRoot);
        boolean check_floor = utils.validate_field(floor, binding.textInputLayoutFloorRoot);
        boolean check_cabinet = utils.validate_field(cabinet, binding.textInputLayoutCabinetRoot);
        boolean check_date_task = utils.validate_field(date_complete, binding.textInputLayoutDateTaskRoot);
        boolean check_executor = utils.validate_field(email_executor, binding.textInputLayoutExecutorRoot);
        boolean check_status = utils.validate_field(status, binding.textInputLayoutStatusRoot);
        boolean check_name_executor = utils.validate_field(full_name_executor, binding.textInputLayoutFullNameExecutorRoot);
